...
async function commentToPR(message) {
  try {
    ...
    const prNumber = parseInt(prNumberFromInput) || context2.payload.pull_request?.number;
    if (!prNumber) {
    ...
    if (commentIncludes) {
      ...
      for await (const { data: comments } of octokit.paginate.iterator(octokit.rest.issues.listComments, {
        ...
        comment = comments.find((comment2) => comment2?.body?.includes(commentIncludes));
        if (comment)
          break;
      }
      if (comment) {
...
var comment_default = commentToPR;

// src/main.ts
var import_child_process = require("child_process");
var MARKDOWN_REPORT_FILENAME = "depcruise-report.md";
function main() {
  const depcruiseOptionsPath = core2.getInput("depcruise_config");
  const depcruiseBaseDir = core2.getInput("depcruise_base_dir");
  const shouldReportStatusCheck = core2.getBooleanInput("status_check");
  if (import_fs.default.existsSync(depcruiseOptionsPath)) {
  ...
  (0, import_child_process.execSync)(`npx dependency-cruiser --config ${depcruiseOptionsPath} -T markdown -f ${MARKDOWN_REPORT_FILENAME} ${depcruiseBaseDir}`, {
    stdio: "inherit"
  });
  const output = import_fs.default.readFileSync(MARKDOWN_REPORT_FILENAME);
  comment_default(output.toString());
  if (shouldReportStatusCheck) {
    (0, import_child_process.execSync)(`npx dependency-cruiser --config ${depcruiseOptionsPath} ${depcruiseBaseDir}`, {
      stdio: "inherit"
    });
  }
}
