...
async function commentToPR(message) {
  try {
    ...
    const pullNumber = parseInt(pr_number) || context2.payload.pull_request?.number;
    if (!pullNumber) {
    ...
    if (commentIncludes) {
      ...
      for await (const { data: comments } of octokit.paginate.iterator(
        ...
        comment = comments.find(
          (comment2) => comment2?.body?.includes(commentIncludes)
        );
        if (comment)
          break;
      }
...
function main() {
  ...
  (0, import_child_process.execSync)(
    `npx depcruise --config ${depcruiseOptionsPath} -T markdown -f depcruise-report.md ${depcruiseBaseDir}`,
    {
      stdio: "inherit"
    }
  );
  const output = import_fs.default.readFileSync(
    import_path.default.resolve(__dirname, "depcruise-report.md")
  );
  const result = output.toString();
  if (process.env.CI) {
    comment_default(result);
  } else {
